...
/* Helper Functions */
static inline int check_overflow(int64_t val, int max) { return val > max; }
static inline int check_underflow(int64_t val, int min) { return val < min; }
...
static inline void send_bounds_error_msg(
    ...
    fprintf(stderr,
        "Error. %s Integer (%d Bits) %s detected. Check your variables. (Values received: %ld & %ld). Defaulting to %ld.\n",
...
static inline nc_i8 nc_new_i8(int64_t val) {
    if (check_overflow(val, INT8_MAX)) {
        send_constructor_error_msg(8, IS_SIGNED, ERROR_OVERFLOW, val, INT8_MAX);
        return (nc_i8){.val = (int8_t)INT8_MAX};
    }

    if (check_underflow(val, INT8_MIN)) {
        send_constructor_error_msg(8, IS_SIGNED, ERROR_UNDERFLOW, val, INT8_MIN);
        return (nc_i8){.val = (int8_t)INT8_MIN};
    }
...
static inline nc_u8 nc_new_u8(int64_t val) {
    if (check_overflow(val, UINT8_MAX)) {
        send_constructor_error_msg(8, IS_UNSIGNED, ERROR_OVERFLOW, val, UINT8_MAX);
        return (nc_u8){.val = (uint8_t)UINT8_MAX};
    }
...
static inline nc_i16 nc_new_i16(int64_t val) {
    if (check_overflow(val, INT16_MAX)) {
        send_constructor_error_msg(16, IS_SIGNED, ERROR_OVERFLOW, val, INT16_MAX);
        return (nc_i16){.val = (int16_t)INT16_MAX};
    }

    if (check_underflow(val, INT16_MIN)) {
        send_constructor_error_msg(16, IS_SIGNED, ERROR_UNDERFLOW, val, INT16_MIN);
        return (nc_i16){.val = (int16_t)INT16_MIN};
    }
...
static inline nc_u16 nc_new_u16(int64_t val) {
    if (check_overflow(val, UINT16_MAX)) {
        send_constructor_error_msg(16, IS_UNSIGNED, ERROR_OVERFLOW, val, UINT16_MAX);
        return (nc_u16){.val = (uint16_t)UINT16_MAX};
    }
...
static inline nc_i32 nc_new_i32(int64_t val) {
    if (check_overflow(val, INT32_MAX)) {
        send_constructor_error_msg(32, IS_SIGNED, ERROR_OVERFLOW, val, INT32_MAX);
        return (nc_i32){.val = (int32_t)INT32_MAX};
    }

    if (check_underflow(val, INT32_MIN)) {
        send_constructor_error_msg(32, IS_SIGNED, ERROR_UNDERFLOW, val, INT32_MIN);
...
static inline nc_u32 nc_new_u32(int64_t val) {
    if (check_overflow(val, UINT32_MAX)) {
        send_constructor_error_msg(32, IS_UNSIGNED, ERROR_OVERFLOW, val, UINT32_MAX);
        return (nc_u32){.val = (uint32_t)UINT32_MAX};
    }
...
static inline nc_i8 nc_checked_add_i8(nc_i8 number_one, nc_i8 number_two) {
    ...
    if (check_overflow(sum, INT8_MAX)) {
        send_bounds_error_msg(8, IS_SIGNED, (int64_t)number_one.val, (int64_t)number_two.val, ERROR_OVERFLOW, INT8_MAX);
        return (nc_i8){.val = (int8_t)INT8_MAX};
    }

    if (check_underflow(sum, INT8_MIN)) {
        send_bounds_error_msg(8, IS_SIGNED, (int64_t)number_one.val, (int64_t)number_two.val, ERROR_UNDERFLOW, INT8_MIN);
        return (nc_i8){.val = (int8_t)INT8_MIN};
    }
...
static inline nc_u8 nc_checked_add_u8(nc_u8 number_one, nc_u8 number_two) {
    ...
    if (check_overflow(sum, UINT8_MAX)) {
        send_bounds_error_msg(8, IS_UNSIGNED, (int64_t)number_one.val, (int64_t)number_two.val, ERROR_OVERFLOW, UINT8_MAX);
        return (nc_u8){.val = (uint8_t)UINT8_MAX};
    }
...
static inline nc_i16 nc_checked_add_i16(nc_i16 number_one, nc_i16 number_two) {
    ...
    if (check_overflow(sum, INT16_MAX)) {
        send_bounds_error_msg(16, IS_SIGNED, (int64_t)number_one.val, (int64_t)number_two.val, ERROR_OVERFLOW, INT16_MAX);
        return (nc_i16){.val = (int16_t)INT16_MAX};
    }

    if (check_underflow(sum, INT16_MIN)) {
        send_bounds_error_msg(16, IS_SIGNED, (int64_t)number_one.val, (int64_t)number_two.val, ERROR_UNDERFLOW, INT16_MIN);
        return (nc_i16){.val = (int16_t)INT16_MIN};
    }
...
static inline nc_u16 nc_checked_add_u16(nc_u16 number_one, nc_u16 number_two) {
    ...
    if (check_overflow(sum, UINT16_MAX)) {
        send_bounds_error_msg(16, IS_UNSIGNED, (int64_t)number_one.val, (int64_t)number_two.val, ERROR_OVERFLOW, UINT16_MAX);
        return (nc_u16){.val = (uint16_t)UINT16_MAX};  
    }
...
static inline nc_u32 nc_checked_add_u32(nc_u32 number_one, nc_u32 number_two) {
    ...
    if (check_overflow(sum, UINT32_MAX)) {
        send_bounds_error_msg(32, IS_UNSIGNED, (int64_t)number_one.val, (int64_t)number_two.val, ERROR_OVERFLOW, UINT32_MAX);
        return (nc_u32){.val = (uint32_t)UINT32_MAX};  
    }
