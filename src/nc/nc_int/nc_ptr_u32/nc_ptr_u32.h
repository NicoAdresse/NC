...
/* Checked Modulo -> nc_ptr_u32 */
static inline nc_ptr_u32 nc_checked_mod_ptr_u32(nc_ptr_u32 ptr_num, nc_u32 val, int is_zero_denominator_allowed) {
    if (!ptr_num.ptr) {
        send_null_pointer_error_msg(IS_UNSIGNED, "nc_ptr_u32");
        return (nc_ptr_u32){.ptr = NULL};
    }
    ...
    uint32_t rem = number_one.val % val.val;
    return nc_new_ptr_u32((nc_u32){.val = rem});
}
...
static inline nc_u32 nc_signum_ptr_u32(nc_ptr_u32 ptr) {
    ...
    return (nc_u32){.val = (addr > 0) - (addr < 0)};
}
